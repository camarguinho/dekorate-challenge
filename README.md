...
NOTE: Dekorate will NOT push images on its own. If the Image Build Strategy is configured for using Docker, it will delegate to the docker binary. So the user needs to make sure beforehand they are logged in and have taken all necessary actions for a docker push to work.

:warning::warning::warning: **If you use Codeready Workspaces, it will not be possible to use Docker as Image Build Strategy because `docker` is not available in this environment.**
